...
function pacificAtlantic(heights) {
  ...
  // BFS solution
  function bfs(rowIndex, colIndex, ocean) {
    ...
    while (queue.length) {
      ...
      for (const [i, j] of directions) {
        const neighborCellRow = row + i,
          neighborCellCol = col + j;
        if (
          neighborCellCol >= 0 &&
          neighborCellCol < columns &&
          neighborCellRow >= 0 &&
          neighborCellRow < rows &&
          heights[neighborCellRow][neighborCellCol] >= heights[row][col] &&
          ocean[neighborCellRow][neighborCellCol] === false
        ) {
          queue.push([neighborCellRow, neighborCellCol]);
        }
      }
    }
  }
  ...
  // DFS solution
  function dfs(rowIndex, colIndex, ocean) {
    ...
    for (const [i, j] of directions) {
      const neighborCellRow = rowIndex + i,
        neighborCellCol = colIndex + j;
      if (
        neighborCellCol >= 0 &&
        neighborCellCol < columns &&
        neighborCellRow >= 0 &&
        neighborCellRow < rows &&
        heights[neighborCellRow][neighborCellCol] >=
          heights[rowIndex][colIndex] &&
        ocean[neighborCellRow][neighborCellCol] === false
      ) {
        dfs(neighborCellRow, neighborCellCol, ocean);
      }
    }
  }
