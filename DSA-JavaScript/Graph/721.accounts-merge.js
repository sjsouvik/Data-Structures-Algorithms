...
const accountsMerge = function (accounts) {
  ...
  const emailToAccount = {}; // email -> index of account
  for (let i = 0; i < n; i++) {
    for (let j = 1; j < accounts[i].length; j++) {
      const email = accounts[i][j];
      if (emailToAccount[email] === undefined) {
  ...
  const emailGroup = {};
  for (const email of Object.keys(emailToAccount)) {
    const accountIndex = unionFind.find(emailToAccount[email]);
    if (emailGroup[accountIndex] === undefined) {
      emailGroup[accountIndex] = [];
    }

    emailGroup[accountIndex].push(email);
  }

  const result = [];

  for (const accountIndex of Object.keys(emailGroup)) {
    result.push([
      accounts[accountIndex][0],
      ...emailGroup[accountIndex].sort(),
    ]);
  }
