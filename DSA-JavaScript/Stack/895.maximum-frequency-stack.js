...
class FreqStack {
  ...
  push(val) {
    const count = (this.countMap[val] || 0) + 1;
    this.countMap[val] = count;
    this.maxCount = Math.max(this.maxCount, count);

    if (this.stacks[count] === undefined) {
      this.stacks[count] = [];
    }
  ...
  pop() {
    const removedItem = this.stacks[this.maxCount].pop();
    this.countMap[removedItem] -= 1;
