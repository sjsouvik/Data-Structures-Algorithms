...
const rob = function (nums) {
  ...
  const util = (index = 0) => {
    ...
    /* the robber has 2 options - either rob the current house and skip the next house or skip the current 
    house and rob the next house. We need to find out the max robbed amount from these 2 options */
    cache[index] = Math.max(nums[index] + util(index + 2), util(index + 1));
    return cache[index];
  };
